      ...
      <TableHeader>
        {table.getHeaderGroups().map((headerGroup) => <TableRow key={headerGroup.id}>{headerGroup.headers.map((header) => {
          ...
          if (!header.isPlaceholder) content = header.column.getCanSort()
            ? <button className={cn("inline-flex w-full items-center gap-1.5 py-2 text-left transition-colors hover:text-foreground", config.align === "right" && "justify-end text-right")} title={`按${config.label}排序`} onClick={header.column.getToggleSortingHandler()}><span className="truncate">{config.label}</span>{sortingIcon(sorted)}</button>
            : <span className={cn("block py-2", config.align === "right" && "text-right")}>{config.label}</span>;

...
export default function BacktestControlsPanel({ activeWorkflow, catalog, displayedParameters, displayedState, displayedWorkflowInstanceId, onFeeAnalysis, onSensitivity, project, projectId, readOnly, ready, selectedVersion, stopping, submitting, summary, workflowState, versions, onCompare, onContinue, onLogs, onParameters, onRun, onSave, onShowParameters, onStop, onValidity, onVersion }: BacktestControlsPanelProps) {
  return <section className="h-full min-h-0 min-w-0"><div className="h-full overflow-y-auto">
    ...
    <div className="space-y-5 p-5"><BacktestEditor catalog={catalog} parameters={displayedParameters} projectId={projectId} readOnly={readOnly} onChange={onParameters} onValidityChange={onValidity} /><div className="flex gap-3">{readOnly ? <Button className="min-w-0 flex-1" onClick={onContinue}><Code2 />基于此版本回测</Button> : <><WorkflowRunButton className="min-w-0 flex-1" active={activeWorkflow} disabled={!ready} label="执行回测" stopping={stopping} submitting={submitting} onRun={onRun} onStop={onStop} /><Button className="min-w-0 flex-1" variant="outline" disabled={!summary || workflowState !== "SUCCESS"} onClick={onSave}><Save />保存</Button></>}<AnalysisMoreActions feeAnalysisDisabled={!readOnly || selectedVersion === null} onFeeAnalysis={onFeeAnalysis} onSensitivity={onSensitivity} sensitivityDisabled={!readOnly || selectedVersion === null} onLogs={onLogs} onShowParameters={onShowParameters} workflowInstanceId={displayedWorkflowInstanceId} /></div></div>
  </div></section>;
}

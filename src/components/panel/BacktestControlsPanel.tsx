...
type BacktestControlsPanelProps = {
  ...
  displayedParameters: BacktestParameters | null;
...
export default function BacktestControlsPanel({ activeWorkflow, catalog, displayedParameters, displayedState, displayedWorkflowInstanceId, onFeeAnalysis, onOptimization, onSensitivity, parameterError, project, projectId, queueCount, readOnly, ready, selectedVersion, stopping, submitting, workflowState, versions, onCompare, onContinue, onDeleteVersion, onLogs, onOpenQueue, onParameters, onQueue, onRenameProject, onRenameVersion, onRun, onSave, onShowParameters, onStop, onValidity, onVersion }: BacktestControlsPanelProps) {
  ...
  return <section className="h-full min-h-0 min-w-0"><div className="h-full overflow-y-auto">
    ...
    <div className="space-y-5 p-5">{displayedParameters
      ? <BacktestEditor catalog={catalog} editorScope={readOnly ? `version-${selectedVersion}` : `draft-${project.draft.id}`} parameters={displayedParameters} projectId={projectId} readOnly={readOnly} onChange={onParameters} onValidityChange={onValidity} />
      ...
      <div className="flex gap-3">{readOnly ? <Button className="min-w-0 flex-1" disabled={!displayedParameters} onClick={onContinue}><Code2 />基于此版本回测</Button> : <><WorkflowExecutionButton className="min-w-0 flex-1" active={activeWorkflow} disabled={!ready} label="执行回测" stopping={stopping} submitting={submitting} onRun={onRun} onStop={onStop} /><Button className="min-w-0 flex-1" variant="outline" disabled={workflowState !== "SUCCESS"} onClick={onSave}><Save />保存</Button></>}<AnalysisMoreActions feeAnalysisDisabled={researchDisabled} optimizationDisabled={researchDisabled} onFeeAnalysis={onFeeAnalysis} onOptimization={onOptimization} onSensitivity={onSensitivity} queueCount={queueCount} queueDisabled={readOnly || !ready} sensitivityDisabled={researchDisabled} researchDisabledReason={snapshot ? "真实快照暂不支持手续费分析、参数敏感性和参数调优；已有报告仍可查看。" : undefined} onLogs={onLogs} onOpenQueue={onOpenQueue} onQueue={onQueue} onShowParameters={onShowParameters} workflowInstanceId={displayedWorkflowInstanceId} /></div></div>
  </div></section>;
}

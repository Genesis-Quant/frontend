...
import { hasCompleteReturnSpecs, type FactorAnalysisParameters } from "@/types/factor";

type FactorAnalysisResultsPanelProps = { displayedParameters: FactorAnalysisParameters; displayedState: string; displayedWorkflowInstanceId: number | null; error: string; readOnly: boolean; running: boolean; workflowError: string | null };

export default function FactorAnalysisResultsPanel({ displayedParameters, displayedState, displayedWorkflowInstanceId, error, readOnly, running, workflowError }: FactorAnalysisResultsPanelProps) {
  ...
  else if (phase === "success" && !hasCompleteReturnSpecs(displayedParameters)) content = <ErrorPanel message="该历史结果缺少可精确识别的收益口径；项目参数仍可查看，但收益报告不会使用猜测值计算。" size="xs" />;
  else if (phase === "success" && factor && displayedWorkflowInstanceId) content = <FactorAnalysisReport factor={factor} key={displayedWorkflowInstanceId} parameters={displayedParameters} workflowInstanceId={displayedWorkflowInstanceId} />;
  else content = !readOnly ? <EmptyStatePanel description="填写左侧参数和 DSL 后执行分析。" icon={FileClock} title="尚未运行分析" /> : null;
  return <section className="min-w-0 space-y-5"><div className="sticky top-20 z-30"><Tabs value={factor} onValueChange={setFactor}><TabsList scrollable>{displayedParameters.factor_columns.map((column) => <TabsTrigger key={column} value={column}>{column}</TabsTrigger>)}</TabsList></Tabs></div>{error && phase === "success" ? <ErrorPanel message={error} size="xs" /> : null}{content}</section>;
}

...
import { terminalStates, type WorkflowApplication, type WorkflowAttemptListPage, type WorkflowAttemptSummary, type WorkflowTaskInformation, type WorkflowWorkspaceListItem, type WorkflowWorkspaceListPage } from "@/types/workflow";
...
const submissionActiveStates = new Set(["CREATED", "QUEUED", "SUBMITTING", "SUBMITTED", "AUTO_SAVE_PENDING"]);
...
export default function WorkflowPanel({ onTotalChange, showUsername = false }: { onTotalChange?: (total: number) => void; showUsername?: boolean }) {
  ...
  const containsPendingUpdate = useMemo(() => result?.items.some((workspace) => !terminalStates.has(workspace.current_attempt.state) || submissionActiveStates.has(workspace.current_attempt.submission_state)) ?? false, [result]);
  ...
  const columns = showUsername ? 6 : 5;
  return <div className="space-y-4">
    ...
    <Card className="gap-0 overflow-hidden py-0 shadow-sm"><CardContent className="p-0"><Table className="table-fixed"><TableHeader><TableRow><TableHead className="w-[28%] px-4">工作空间</TableHead>{showUsername ? <TableHead className="w-28">用户名</TableHead> : null}<TableHead className="w-56">当前运行</TableHead><TableHead>任务</TableHead><TableHead className="w-40">时间 / 耗时</TableHead><TableHead className="w-24 px-4 text-right">操作</TableHead></TableRow></TableHeader><TableBody>
      {result?.items.map((workspace) => {
        ...
        return <Fragment key={workspace.workspace_id}>
          <WorkspaceRow canOpenProject={workspace.user_id === userId} expanded={expanded} now={now} showUsername={showUsername} stopping={workspace.current_attempt.workflow_instance_id === stoppingWorkflowId} workspace={workspace} onDetails={() => setDetailsAttemptId(workspace.current_attempt.attempt_id)} onLogs={(taskInstanceId) => { if (workspace.current_attempt.workflow_instance_id !== null) setSelectedTask({ workflowInstanceId: workspace.current_attempt.workflow_instance_id, taskInstanceId }); }} onStop={() => { if (workspace.current_attempt.workflow_instance_id !== null) stopWorkflow(workspace.current_attempt.workflow_instance_id); }} onToggle={() => toggleWorkspace(workspace.workspace_id)} />
          {expanded ? <WorkspaceAttemptsRow attemptPage={attemptsByWorkspace[workspace.workspace_id]} columns={columns} error={attemptErrors[workspace.workspace_id]} loading={loadingWorkspaceIds.has(workspace.workspace_id)} now={now} stoppingWorkflowId={stoppingWorkflowId} workspace={workspace} onDetails={setDetailsAttemptId} onLoadMore={() => loadAttempts(workspace.workspace_id, (attemptsByWorkspace[workspace.workspace_id]?.page ?? 0) + 1)} onRetry={() => loadAttempts(workspace.workspace_id)} onStop={stopWorkflow} /> : null}
        </Fragment>;
      })}
...
function WorkspaceRow({ canOpenProject, expanded, now, onDetails, onLogs, onStop, onToggle, showUsername, stopping, workspace }: { canOpenProject: boolean; expanded: boolean; now: number; onDetails: () => void; onLogs: (taskInstanceId: number) => void; onStop: () => void; onToggle: () => void; showUsername: boolean; stopping: boolean; workspace: WorkflowWorkspaceListItem }) {
  ...
  return <TableRow aria-expanded={expanded} className={cn(expanded && "border-b-0 bg-muted/25")}>
    <TableCell className="whitespace-normal px-4 py-3"><div className="flex min-w-0 items-start gap-2"><Button aria-label={expanded ? "收起运行记录" : "展开运行记录"} aria-expanded={expanded} className="mt-0.5 size-7 shrink-0" size="icon-sm" variant="ghost" onClick={onToggle}>{expanded ? <ChevronDown /> : <ChevronRight />}</Button><div className="min-w-0"><div className="flex min-w-0 flex-wrap items-center gap-2"><WorkflowApplicationBadge application={workspace.application} linkToProject={canOpenProject} projectId={workspace.project_id} /><span className="min-w-0 truncate text-sm font-medium" title={workspace.project_title ?? undefined}>{workspace.project_title ?? "工作流任务"}</span></div><div className="mt-1.5 font-mono text-[10px] text-muted-foreground">工作空间 #{workspace.workspace_id} · {workspace.attempt_count} 次运行</div></div></div></TableCell>
    {showUsername ? <TableCell className="truncate text-sm font-medium" title={workspace.owner_username}>{workspace.owner_username}</TableCell> : null}
    <TableCell className="whitespace-normal py-3"><div className="flex flex-wrap items-center gap-2"><SchedulerStateBadge state={attempt.state} />{attempt.is_current ? <Badge variant="secondary">当前</Badge> : null}</div><div className="mt-1.5 text-xs font-medium">第 {attempt.attempt_number} 次运行</div><div className="mt-1 truncate font-mono text-[10px] text-muted-foreground" title={attempt.workflow_name ?? undefined}>Attempt #{attempt.attempt_id}{attempt.workflow_instance_id === null ? " · 尚未创建 Workflow" : ` · Workflow #${attempt.workflow_instance_id}`}</div></TableCell>
    <TableCell className="whitespace-normal py-3"><WorkflowTaskCapsules tasks={attempt.tasks} error={attempt.tasks_error} onLogs={onLogs} /></TableCell>
    <TableCell className="whitespace-normal py-3"><WorkflowStartTime value={attempt.started_at ?? attempt.created_at} /><span className="mt-1.5 inline-flex items-center gap-1.5 font-mono text-xs"><Clock3 className="size-3 text-muted-foreground" />{formatDuration(duration)}</span></TableCell>
    <TableCell className="px-4"><div className="flex justify-end gap-1">{active && attempt.workflow_instance_id !== null ? <Button title={stopping ? "正在终止工作流" : "终止工作流"} aria-label={stopping ? "正在终止工作流" : "终止工作流"} size="icon-sm" variant="destructive" disabled={stopping} onClick={onStop}>{stopping ? <Loader2 className="animate-spin" /> : <Square />}</Button> : null}<Button title="查看本次运行详情" aria-label="查看本次运行详情" size="icon-sm" variant="ghost" onClick={onDetails}><Eye /></Button></div></TableCell>
  </TableRow>;
}

function WorkspaceAttemptsRow({ attemptPage, columns, error, loading, now, onDetails, onLoadMore, onRetry, onStop, stoppingWorkflowId, workspace }: { attemptPage?: WorkflowAttemptListPage; columns: number; error?: string; loading: boolean; now: number; onDetails: (attemptId: number) => void; onLoadMore: () => void; onRetry: () => void; onStop: (workflowInstanceId: number) => void; stoppingWorkflowId: number | null; workspace: WorkflowWorkspaceListItem }) {
  const attempts = attemptPage?.items;
  const hasMore = attemptPage !== undefined && attemptPage.items.length < attemptPage.total;
  return <TableRow className="border-b bg-muted/10 hover:bg-muted/10"><TableCell className="whitespace-normal p-0" colSpan={columns}><div className="border-t px-4 py-4 sm:px-6"><div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-xs font-semibold">运行记录</h3><p className="mt-1 text-[11px] text-muted-foreground">工作空间 #{workspace.workspace_id} 的全部提交与执行结果</p></div><Badge variant="outline">{workspace.attempt_count} 次</Badge></div>
    {loading && !attempts ? <div className="flex min-h-24 items-center justify-center gap-2 text-xs text-muted-foreground"><Loader2 className="size-4 animate-spin" />正在读取运行记录...</div> : null}
    ...
    {attempts?.length ? <div className="divide-y rounded-md border bg-background">{attempts.map((attempt) => <AttemptHistoryItem attempt={attempt} key={attempt.attempt_id} now={now} stopping={attempt.workflow_instance_id === stoppingWorkflowId} onDetails={() => onDetails(attempt.attempt_id)} onStop={() => { if (attempt.workflow_instance_id !== null) onStop(attempt.workflow_instance_id); }} />)}</div> : null}
    ...
    {!loading && !error && attempts && !attempts.length ? <div className="grid min-h-20 place-items-center text-xs text-muted-foreground">暂无运行记录</div> : null}
...
function AttemptHistoryItem({ attempt, now, onDetails, onStop, stopping }: { attempt: WorkflowAttemptSummary; now: number; onDetails: () => void; onStop: () => void; stopping: boolean }) {
  const active = workflowIsRunning(attempt);
  const duration = resolveDurationSeconds(attempt.duration_seconds, attempt.started_at, attempt.finished_at, active, now);
  return <div className="grid gap-3 px-4 py-3 lg:grid-cols-12 lg:items-center">
    <div className="min-w-0 lg:col-span-3"><div className="flex flex-wrap items-center gap-2"><SchedulerStateBadge state={attempt.state} />{attempt.is_current ? <Badge variant="secondary">当前</Badge> : null}</div><div className="mt-1.5 text-xs font-medium">第 {attempt.attempt_number} 次运行</div><div className="mt-1 truncate font-mono text-[10px] text-muted-foreground">Attempt #{attempt.attempt_id}</div></div>
    <div className="min-w-0 lg:col-span-3"><div className="truncate text-xs font-medium" title={attempt.workflow_name ?? undefined}>{attempt.workflow_name ?? "尚未创建调度实例"}</div><div className="mt-1 truncate font-mono text-[10px] text-muted-foreground">{attempt.workflow_instance_id === null ? "Workflow —" : `Workflow #${attempt.workflow_instance_id}`}{attempt.workflow_definition_code === null ? "" : ` · Definition #${attempt.workflow_definition_code}`}</div><div className="mt-1 text-[10px] text-muted-foreground">更新于 {formatDateTime(attempt.updated_at)}</div></div>
    <div className="min-w-0 lg:col-span-4"><div className="flex flex-wrap items-center gap-2"><span className="text-[10px] text-muted-foreground">提交状态</span><SchedulerStateBadge state={attempt.submission_state} /></div><div className="mt-2 flex flex-wrap gap-1.5">{attempt.requested_outputs.length ? attempt.requested_outputs.map((output) => <Badge className="font-mono text-[10px]" key={output} variant="outline">{output}</Badge>) : <span className="text-[10px] text-muted-foreground">未指定输出</span>}</div>{attempt.error ? <p className="mt-2 line-clamp-2 text-[10px] leading-4 text-destructive" title={attempt.error}>{attempt.error}</p> : null}</div>
    <div className="flex items-center justify-between gap-3 lg:col-span-2 lg:justify-end"><div className="min-w-0 text-right"><WorkflowStartTime value={attempt.started_at ?? attempt.created_at} /><span className="mt-1 inline-flex items-center gap-1 font-mono text-[11px]"><Clock3 className="size-3 text-muted-foreground" />{formatDuration(duration)}</span></div><div className="flex shrink-0 gap-1">{active && attempt.workflow_instance_id !== null ? <Button title="终止工作流" aria-label="终止工作流" size="icon-sm" variant="destructive" disabled={stopping} onClick={onStop}>{stopping ? <Loader2 className="animate-spin" /> : <Square />}</Button> : null}<Button title="查看本次运行详情" aria-label="查看本次运行详情" size="icon-sm" variant="ghost" onClick={onDetails}><Eye /></Button></div></div>
  </div>;
}
...
function WorkflowTaskCapsules({ error, onLogs, tasks }: { error?: string | null; onLogs: (taskInstanceId: number) => void; tasks: WorkflowTaskInformation[] }) {

...
type BacktestResultsPanelProps = { annualTradingDays: number; displayedState: string; displayedWorkflowInstanceId: number | null; error: string; readOnly: boolean; riskFreeRate: number; running: boolean; workflowError: string | null };
...
export default function BacktestResultsPanel({ annualTradingDays, displayedState, displayedWorkflowInstanceId, error, readOnly, riskFreeRate, running, workflowError }: BacktestResultsPanelProps) {
  ...
  else if (phase === "success" && displayedWorkflowInstanceId) content = <BacktestReport annualTradingDays={annualTradingDays} key={displayedWorkflowInstanceId} riskFreeRate={riskFreeRate} workflowInstanceId={displayedWorkflowInstanceId} />;

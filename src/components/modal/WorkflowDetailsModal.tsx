...
function ParameterTabs({ attemptId, payload }: { attemptId: number; payload: WorkflowAttemptInformation["payload"] }) {
  return <Tabs className="mt-3" defaultValue="start"><TabsList><TabsTrigger value="start">工作流参数</TabsTrigger><TabsTrigger value="input">Input JSON</TabsTrigger></TabsList><TabsContent value="start"><JsonParameters modelPath={`json://workflow-attempt/${attemptId}/start-parameters.json`} value={payload.start_parameters} /></TabsContent><TabsContent value="input"><JsonParameters modelPath={`json://workflow-attempt/${attemptId}/input.json`} value={payload.input_json} /></TabsContent></Tabs>;
}
function JsonParameters({ modelPath, value }: { modelPath: string; value: Record<string, unknown> }) { return <JsonEditor ariaLabel="只读 JSON 参数" autoHeight className="mt-3" maxVisibleLines={16} minVisibleLines={5} modelPath={modelPath} readOnly value={JSON.stringify(value, null, 2)} />; }

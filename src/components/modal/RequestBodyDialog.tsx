  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }}>
    <DialogContent className="flex h-[82vh] max-h-[860px] flex-col overflow-hidden p-0 sm:max-w-4xl">
      ...
      <div className="min-h-0 flex-1 p-4">
        <JsonEditor ariaLabel="完整 JSON 请求体" modelPath={`json://request-body${endpoint}.json`} readOnly={!editable} value={source} onChange={(nextSource) => { setSource(nextSource); setError(""); }} />
      </div>

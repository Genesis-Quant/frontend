...
export default function AppLayout({ children, resolvePrimaryPath }: { children: ReactNode; resolvePrimaryPath: (path: string) => string }) {
    ...
    <header className="sticky top-0 z-40 border-b border-border/35 bg-background/80 backdrop-blur-xl lg:border-border lg:bg-background/95">
      <div className="mx-auto flex min-h-16 max-w-[1440px] flex-wrap items-center justify-between gap-3 px-3 py-2 sm:px-6 lg:h-16 lg:flex-nowrap lg:gap-0 lg:py-0">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <Button className="border-border/40 bg-background/35 lg:bg-background" size="icon" variant={active === "profile" ? "default" : "outline"} asChild><Link to={resolvePrimaryPath("/profile")}><Avatar className="size-7"><AvatarFallback>{user?.username.slice(0, 1).toUpperCase()}</AvatarFallback></Avatar><span className="sr-only">{user?.username}</span></Link></Button>
        </div>
      </div>
    </header>
    {active === "home"

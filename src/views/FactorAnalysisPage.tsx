...
export default function FactorAnalysisPage() {
  ...
  return <div className="space-y-5">
    <PageHero chips={["多因子分析", "版本迭代", "横向对比"]} description="管理因子分析项目、当前草稿和已保存版本，继续迭代同一项研究。" eyebrow="FACTOR ANALYSIS" icon={FlaskConical} stat={{ label: "研究项目", value: projects?.total ?? 0 }} title="因子分析" variant="analysis" />

    <ProjectListActions createLabel="新建分析" loading={loading} onCreate={() => setCreateOpen(true)} onRefresh={load}><Button variant="outline" disabled={(projects?.items.length ?? 0) === 0} onClick={() => setCompareOpen(true)}><GitCompare />对比研究</Button></ProjectListActions>
